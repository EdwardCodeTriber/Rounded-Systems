...
app.get('http://localhost:5000/api/employees', async (req, res) => {
  try {
...
app.post('http://localhost:5000/api/employees', async (req, res) => {
  try {
    const { id, name, surname, position, email, idNumber, picture } = req.body;
    
    // Validate required fields
    if (!id || !name || !surname || !position || !email || !idNumber) {
...
app.put('http://localhost:5000/api/employees/:id', async (req, res) => {
  const { id } = req.params;
  const { name, surname, position, email, idNumber, picture } = req.body;
  try {
    const employeeRef = db.collection('employees').doc(id);
    const updateData = { name, surname, position, email, idNumber, picture, updatedAt: admin.firestore.FieldValue.serverTimestamp() };
    ...
    if (updatedEmployee.updatedAt) {
      updatedEmployee.updatedAt = updatedEmployee.updatedAt.toDate().toISOString();
    }
...
app.delete('http://localhost:5000/api/employees/:id', async (req, res) => {
  const { id } = req.params;
  try {
